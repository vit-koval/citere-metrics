...
**#/priorities** — preview is three scannable tables, detail is everything. Recommendations: topic, demand, gap, dominant cause with confidence, priority, potential (the best action's expected range), task count and task status summary. Healthy: topic, demand, gap, dominant code, failure-code share. Label/Medical: topic, demand, gap, cause, sign-off status. Every column sorts and the chosen sort travels in the hash; default is priority then potential. A row opens the topic's detail page: a potential panel in plain language, the three numbers as a sentence, the cause with confidence, owner tags carrying lever and ceiling (no repeated action text), top sources, what happened and why, the point list by rank score, and then every individual task belonging to the topic as the same cards the Action Center renders, with a link that carries the filter over. Original shape: three sub-lists: Recommendations (groups, one row per group, owners as sub-rows with lever and ceiling), Healthy groups (with `failure_code_share`), Label / Medical rows. Each group row shows: three numbers (demand, gap with dominant run, lever), dominant code as title, dominant cause as sub-line, reach ceiling. Click a group → `#/priorities?group=<id>` — the point list of that group ordered by `point_rank_score`.
