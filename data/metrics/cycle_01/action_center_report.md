...
Counters (client tasks, execution auto/manual): open 60 (auto 16, manual 44); in progress 0; done this cycle 0; dismissed 0. By who: Digital 26, Marketing + Medical 3, Medical + Digital 6, PR/Comms 9, Medical 6, Comms 6, Medical Affairs 1, Regulatory 3.
Citere tasks (own monitoring / re-check, excluded from client counters): 3. Label / Medical tasks held out of the queue while their findings await clinician sign-off: 2 (10 findings pending in label_findings_registry.json).
...
Registry: `data/registry/action_center_tasks.json` — 60 client + 3 citere + 2 held label tasks; task_id = sha1(group_id|owner)[:12]; status/dismiss_reason/cycle fields are never overwritten by computation.
Outputs: action_center_queue.csv (63 rows, open first then by score), action_center_summary.json.
